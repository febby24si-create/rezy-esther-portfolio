...
function InspectionPanel({ order, onUpdate }) {
  ...
  function handleStart() {
    setLoading(true)
    const result = startInspection(order.id, { startedBy: 'Admin' })
    setLoading(false)
  ...
  function handleComplete() {
    ...
    setLoading(true)
    const result = completeInspection(order.id, { items, notes, by: 'Admin' })
    setLoading(false)
...
function EstimasiPanel({ order, onUpdate }) {
  ...
  function handleSave() {
    ...
    setLoading(true)
    const result = saveEstimation(order.id, { items, laborCost: Number(laborCost), discount: Number(discount), notes, by: 'Admin' })
    setLoading(false)
...
function ApprovalPanel({ order, onUpdate }) {
  ...
  function handleApprove() {
    setLoading(true)
    const result = approveEstimation(order.id, { approvedBy: 'Customer', notes: 'Disetujui via portal' })
    setLoading(false)
  ...
  function handleRevision() {
    ...
    setLoading(true)
    const result = requestEstimationRevision(order.id, { reason, by: 'Customer' })
    setLoading(false)
...
function QCPanel({ order, onUpdate }) {
  ...
  function handleStart() {
    setLoading(true)
    const result = startQC(order.id, { by: 'Admin' })
    setLoading(false)
  ...
  function handleComplete(passed) {
    ...
    setLoading(true)
    const result = completeQC(order.id, { items, passed, notes, by: 'Admin' })
    setLoading(false)
    if (result.success) {
      onUpdate()
...
export default function OrderDetail() {
  ...
  const loadOrder = useCallback(() => {
    try {
      ...
      const found = all.find(o => o.id === id || o.id === decodeURIComponent(id))
      if (found) { setOrder(found); setLoading(false) }
  ...
  useEffect(() => {
    loadOrder()
    const iv = setInterval(loadOrder, 2000)
